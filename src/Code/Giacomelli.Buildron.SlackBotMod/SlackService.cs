	...
	public class SlackService
	{
		...
		bool IsMessageToBot(SlackResponse response)
		{
			return response != null
				&& response.type.Equals("message") && !"file_share".Equals(response.subtype)
				&& (
		...
		public void UploadFile(string filePath)
		{
			...
			m_log.Debug("Enqueuing file upload {0}...", fileName);

			UnityMainThreadDispatcher.Instance().Enqueue(() =>
			{
				m_log.Debug("Uploading file {0}...", fileName);
				var url = "https://slack.com/api/files.upload";
				var data = new WWWForm();
				data.AddField("token", m_slackKey);
				data.AddField("filename", fileName);
				data.AddField("channels", m_messageToRespond.Channel );
				data.AddField("username", "Buildron");
				data.AddField("icon_url", "https://raw.githubusercontent.com/skahal/Buildron/master/docs/images/Buildron-logo-128x128.png");
				data.AddBinaryData("file", File.ReadAllBytes(filePath), fileName);
