...
{
	public class FiterByMessageHandler : MessageHandlerBase
	{
		...
		public FiterByMessageHandler(IModContext modContext, SlackService slackService)
			: base(modContext, slackService)
		{
		}

		public override bool Process(SlackMessage message)
		{
			...
			if (match.Success)
			{
				...
				Slack.Send("Ok, take a look on my screen right now! I'm filtering builds by {0} '{1}'. ", kind, value);
